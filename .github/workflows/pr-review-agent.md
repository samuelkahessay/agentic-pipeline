...
6. **Check CI status** — run `gh aw checks <PR_NUMBER> --json`.
   - Treat the returned `state` field as the authoritative CI verdict. Do NOT reinterpret raw check runs or commit statuses yourself.
...
8. **Decision rules**:
   - **APPROVE** if: all acceptance criteria are met, no obvious bugs or security issues, code is within scope, and CI `state` is not `failed` or `policy_blocked`
   - **REQUEST_CHANGES** if: any acceptance criteria are NOT met, there are bugs/security issues, the PR is clearly out of scope, CI `state` is `failed`, or CI `state` is `policy_blocked`
