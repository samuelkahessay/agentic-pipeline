...
test("demo session creation replaces a malformed encryption key", async () => {
  process.env.ENCRYPTION_KEY = "bad-key";
  ...
  expect(process.env.ENCRYPTION_KEY).toMatch(/^[a-f0-9]{64}$/);
  expect(process.env.ENCRYPTION_KEY).not.toBe("bad-key");
  const grantCount = db
    .prepare("SELECT COUNT(*) AS count FROM oauth_grants")
    .get().count;
  const demoBuildCount = db
